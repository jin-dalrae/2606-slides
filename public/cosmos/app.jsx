...
// Six clusters: product sides around App, Industry across the top (large ring).
const sideAnchors = {
  app: { x: 800, y: 640 },
  users: { x: 280, y: 400 },
  writers: { x: 280, y: 920 },
...
// Larger entity spacing for readable labels without center bubbles.
const CLUSTER_MIN_CHORD = 118;
const CLUSTER_MIN_RADIUS = 105;

// Precomputed ring angles: entities face their influence partners so total
// center-to-center edge length is minimized (~18% below even spacing).
// Recompute if networkSides / influenceEdges change substantially.
const nodeRingAngles = {
  // users
...
function placeCluster(side) {
  ...
  const n = Math.max(side.nodes.length, 1);
  // Fallback: even spacing if an entity is missing a precomputed angle.
  ...
  const nodes = side.nodes.map((node, i) => {
    const angle =
      nodeRingAngles[node.id] ?? fallbackStart + (i / n) * Math.PI * 2;
    return {
      ...node,
      sideId: side.id,
      angle,
        ...
        <div className="stakeholder-frame__map stakeholder-frame__map--page" ref={mapRef}>
          <svg className="stakeholder-map" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
            ...
            <g className="stakeholder-map__camera" style={cameraStyle}>
              ...
              {/* Influence edges: card N/E/S/W anchors so arrowheads sit on the rim */}
