...
/**
 * Place nodes on each ring so they face weighted neighbors (shorten arrows).
 * Deterministic, cheap: sort by neighbor centroid angle, even spacing, best rotation.
 */
function layoutNetworkGraph(sides, edges) {
  ...
  // Seed: even rings facing App (or north for hub).
  for (const side of sides) {
    const anchor = sideAnchors[side.id];
...
const networkGraph = layoutNetworkGraph(networkSides, influenceEdges);
